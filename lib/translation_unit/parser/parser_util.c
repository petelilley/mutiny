...
bool mt_tok_kw_match(mt_token_t* t, mt_keyword_t k, mt_error_reporter_t* e) {
  ...
  if (mt_tok_comp(t, TK_KEYWORD)) {
    mt_report_syntax_error(e, t->file, t->line, t->col, t->len, "Unexpected %s, expected %s", mt_keyword_to_str((mt_keyword_t)(t->ival)), mt_keyword_to_str(k));
  }
  else {
...
bool mt_tok_punct_match(mt_token_t* t, const char* p, mt_error_reporter_t* e) {
  if (mt_tok_punct_comp(t, p)) {
    return true;
  }
  if (mt_tok_comp(t, TK_PUNCTUATOR)) {
    mt_report_syntax_error(e, t->file, t->line, t->col, t->len, "Unexpected `%.*s', expected `%s'", t->len, t->strval, p);
  }
  else {
